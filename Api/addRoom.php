...
if (isset($_POST['code']) && isset($_POST['newRoomName']) && isset($_POST['managerEmail'])) {
    ...
    if ($findRoom->rowCount() != 0) {
        ...
        $addRoom = $dbh->prepare('INSERT INTO river_room (roomID, roomName, managerEmail, roomPassword ) VALUES (?, ?, ?,?)');
        $addRoom->execute(array($_POST['code'], $_POST['newRoomName'], $_POST['managerEmail'], $password));

        // 順便開一個存對話的 table
        $addRoomMsg = $dbh->prepare('INSERT INTO river_message (roomID ) VALUES (?)');
        $addRoomMsg->execute(array($_POST['code']));

        // 再順便開一個存問題的 table
        $addRoomQuestion = $dbh->prepare('INSERT INTO river_question (roomID ) VALUES (?)');
